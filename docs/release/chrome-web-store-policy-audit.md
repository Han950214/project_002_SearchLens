...
| Program Policies | https://developer.chrome.com/docs/webstore/program-policies/policies | 准确元数据；测试无崩溃；单一用途说明；有效联系方式；用户数据合规 | 是 | `wxt.config.ts`、`PRIVACY.md`、`docs/release/`、既有 smoke 结论 | 无；后台填写由用户完成 |
...
| Privacy Policies | https://developer.chrome.com/docs/webstore/program-policies/privacy | 处理任何用户数据时必须提供准确、最新且完整披露收集、使用、共享对象的隐私政策 | 本地稿满足 | `PRIVACY.md`、`docs/privacy/index.md` 与代码数据流一致 | 用户启用并核验 GitHub Pages 公网 URL |
...
| Prepare your extension | https://developer.chrome.com/docs/webstore/prepare | 检查 name/version/icons/description；ZIP 根目录直接含 manifest | 是 | 构建 manifest、图标、最终 ZIP 清单与 SHA-256 | 无 |
...
- 发布者信息：公开开发者名称 SearchLens；支持邮箱 `826124445@qq.com`。
- 未完成用户操作：push、启用并公开核验 GitHub Pages、开发者后台填写、ZIP 上传、提交审核与发布决策。
